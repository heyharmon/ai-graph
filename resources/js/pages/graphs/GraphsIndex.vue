                ...
                <div class="hidden md:block">
                    <table class="min-w-full divide-y divide-neutral-200">
                        ...
                        <tbody class="divide-y divide-neutral-200">
                            ...
                            >
                                <td class="px-4 py-3 text-sm font-medium text-neutral-900">{{ graph.name }}</td>
                                <td class="px-4 py-3 text-sm text-neutral-600">{{ graph.website_url }}</td>
                                <td class="px-4 py-3">
                                    <span
                                        :class="[
                                ...
                                <td class="px-4 py-3">
                                    <Button 
                                        size="sm" 
                                        variant="outline" 
                                        @click.stop="router.push({ name: 'graphs.show', params: { id: graph.id } })"
                                    >
                                        View
                                    </Button>
                                </td>
                            ...
                            <tr v-if="!graphs.length && !loading">
                                <td colspan="5" class="px-4 py-6 text-center text-sm text-neutral-500">No graphs found. Create your first graph to get started.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                    ...
                    >
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex-1 min-w-0">
                                <div class="text-base font-semibold text-neutral-900">{{ graph.name }}</div>
                                <div class="mt-1 text-sm text-neutral-600">{{ graph.website_url }}</div>
                            </div>
                            <span
                                :class="[
                                    'inline-flex rounded-full px-3 py-1 text-xs font-semibold',
